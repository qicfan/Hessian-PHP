...
class HessianCURLTransport implements IHessianTransport
{
    ...
    function testAvailable()
    {
        if (!function_exists('curl_init')) throw new Exception('You need to enable the CURL extension to use the curl transport');
    }
    ...
    {
        $ch = curl_init($url);
        if (!$ch) throw new Exception("curl_init error for url $url.");
        ...
        if (!empty($options->saveRaw)) $this->rawData = $result;
        $this->safeClose($ch);
        if ($code != 200) {
            $this->safeClose($ch);
            $msg = "Server error, returned HTTP code: $code";
            if (!empty($options->saveRaw)) $msg .= " Server sent: " . $result;
...
class HessianHttpStreamTransport implements IHessianTransport
{
    ...
    function testAvailable()
    {
        if (!ini_get('allow_url_fopen')) throw new Exception("You need to enable allow_url_fopen to use the stream transport");
    }
    ...
    function getStream($url, $data, $options)
    {
        ...
        if (!empty($options->transportOptions)) {
            ...
            if (isset($http['header'])) {
                ...
                if (is_array($headers)) $newheaders = $headers;
                if (!empty($newheaders)) {
                    //$params['header'] = array();
            ...
            $params = $params + $http;
            if (isset($http['timeout'])) $params['timeout'] = $http['timeout'];
        }

        $scheme = 'http';
        if (strpos($url, 'https') === 0) $scheme = 'https';
        ...
        fclose($fp);
        if (!empty($options->saveRaw)) $this->rawData = $response;
